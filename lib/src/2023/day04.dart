...
Iterable<Iterable<int>> _getCards(String input) => input.getLines().map(
      (e) => e
          .split(':')
          .last
          .split('|')
...
int day04star1(String input) => _getCards(input)
    .where((element) => element.isNotEmpty)
    .map((e) => pow(2, e.length - 1))
    .sum
    .toInt();

int day04star2(String input) {
  final cards = _getCards(input);
  final winningMultiplier = <int, int>{1: 0};
  for (final card in cards.indexed(offset: 1)) {
