...
int day1star2(Input input) => input
    ...
    .map((e) {
      ...
      for (final digitWord in digitWords) {
        final idx = element.indexOf(digitWord.$1);
        if (idx != -1 && idx < index) {
          first = digitWord;
          index = idx;
        }
      ...
      index = -1;
      for (final digitWord in digitWords) {
        final idx = element.lastIndexOf(digitWord.$1);
        if (idx > index) {
          last = digitWord;
          index = idx;
        }
      ...
      if (first != null) {
        element =
            element.replaceFirst(first.$1, '_${first.$2}_${first.$1}') as Input;
      }
      if (last != null) {
        element =
            element.replaceFirst(last.$1, '${last.$1}_${last.$2}_', index)
                as Input;
      }
      return element;
    })
